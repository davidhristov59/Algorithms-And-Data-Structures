...
public class BTree<E> {
    ...
    int insideNodesR(BNode<E> node) { //funckija sto go dava brojot na vnatresni(neterminalni) jazli vo dadeno binarno drvo
        if (node == null)
            return 0;
    ...
    int leavesR(BNode<E> node) { //funkcija sto go dava brojot na listovi vo dadeno binarno drvo
        if (node != null) {
            if ((node.left == null)&&(node.right == null))
                return 1;
            else
    ...
    int depthR(BNode<E> node) { //najdolgata pateka ke ja vrati
        if (node == null)
            return 0;
        if ((node.left == null)&&(node.right == null))
            return 0;
