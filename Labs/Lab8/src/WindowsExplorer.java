    ...
    public static void main(String[] args) throws Exception {
        ...
        for (i = 0; i < commands.length; i++) {
            ...
            if(parts[0].equals("CREATE")){
                tree.addChild(curr,parts[1]); //dodadi nov directory/file vo current directory
            }
            else if(parts[0].equals("OPEN")){
                curr = (SLLNode<String>) tree.getNode(curr,parts[1]); //se menuva tekovniot folder
            }
            else if(parts[0].equals("DELETE")){
//                tree.remove(curr);
                SLLNode<String> izbrisan = curr.firstChild;
                while (izbrisan != null){
                    if(izbrisan.element.equals(parts[1])){
                        tree.remove(izbrisan);
                    }
                    izbrisan = izbrisan.sibling; //dvizi desno niz drvoto
                }
            }
            else if(parts[0].equals("BACK")){
                curr = curr.parent; //se vraka nazad sto znaci na roditelot se vraka
            }
