    ...
    public class ArrayMeanWordLength {
        ...
        public static String wordClosestToAverageLength(Array<String> arr) {
            ...
            for (int i = 0; i < arr.getSize(); i++) {
                suma += arr.get(i).length();
            }

            average = suma / arr.getSize();



       /* int brojNaZborovi=arr.getSize();
        Array<String> niza=arr;*/
            String wordClosestToAverage = arr.get(0);
            double difference = Math.abs(arr.get(0).length() - average);

            for (int i = 1; i < arr.getSize(); i++) {
                String word = arr.get(i);
                double razlika = Math.abs(word.length() - average);
                if (razlika < difference || (razlika == difference && word.length() > wordClosestToAverage.length())) {
                    wordClosestToAverage = word;
                    difference = razlika;
                }
            }
            return wordClosestToAverage;
        }
